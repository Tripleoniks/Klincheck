 ...
 /* -----------calling the Api------------------ */
            ...
            if (ndlValue.length >=10) {
                ...
                try {
                    ...
                    if(data.status_code === 200) {
                         history.push('/valid', {userData: data?.data?.response, idNumber:ndlValue})
                    } 
                } catch (error) {
   ...
   /*------------------- Verification Button --------------------------------- */ 
        function VerifyButton() {
            if (firstName && lastName && dob && ndlValue && ndlValue.length >=10 && isChecked ) {
    ...
    return ( 
        ...
        <form onSubmit={verify}>
                     ...
                     <div className="row">
                        ...
                        <div className="col-md-6 col-12 text-center"> <input type="text" className="input_area"  value={ndlValue} placeholder="Driver's License No"  onChange={(e)=> setNdlValue(e.target.value)} /> </div>
                        </div>
                        </div>
                        <div className="row">
